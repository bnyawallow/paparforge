...
export function instantiateTemplate(templateId: string, parentId?: string): SceneObject | null {
  ...
  const is2DOverlay = ['hudCanvas', 'hudText', 'hudButton', 'hudImage', 'hudEmbed'].includes(template.objectType);
  let resolvedParentId = parentId || null;

  if (is2DOverlay) {
    if (!resolvedParentId) {
      // Find existing 2D Canvas in objects
      const activeHUDCanvas = Object.values(store.objects).find(o => o.type === 'hudCanvas');
      if (activeHUDCanvas) {
        resolvedParentId = activeHUDCanvas.id;
      } else {
